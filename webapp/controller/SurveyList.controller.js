...
sap.ui.define([
	...
	return Controller.extend("org.fater.app.controller.SurveyList", {
		...
		onListItemPress:function(oEvent){
			...
			if( !isSupplier && (status === "C1" || status === "C2" || status === "C3" || status === "ER" || status === "ES" || status === "QC") ) {
				this.getRouter().navTo("dataCompletion",{id:participationID});
			} else {
				this.getRouter().navTo("detail",{id:participationID});
			}
		},
		...
		onFilterParticipationTypePress: function(oEvent) {
			if (!this._oFilterDialog) {
				this._oFilterDialog = sap.ui.xmlfragment("org.fater.app.view.fragment.dialogs.ParticipationTypeMenu", this);
		...
		applyFilter: function(string, initList){
		    ...
		    var oBinding = oList.getBinding("items");
		    var role = this.getView().getModel("user").getProperty("/Role");
		    var filterValue = role === this.getView().getModel("Types").getProperty("/userRoleTypes/SUPPLIER") ? "ParticipationId" : "Supplier/Name1";
